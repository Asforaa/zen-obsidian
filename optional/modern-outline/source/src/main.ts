...
export default class ModernOutlinePlugin extends Plugin {
	...
	getPreferredMarkdownView(): MarkdownView | null {
		...
		const leaves = this.app.workspace.getLeavesOfType('markdown')
			...
			.sort((a, b) => {
				const aTime = (a as typeof a & { activeTime?: number }).activeTime ?? 0;
				const bTime = (b as typeof b & { activeTime?: number }).activeTime ?? 0;
				return bTime - aTime;
			});
		const selected = leaves.find(leaf =>
			(leaf as typeof leaf & { containerEl: HTMLElement }).containerEl.classList.contains('mod-active')
		);
		return (selected?.view ?? leaves[0]?.view ?? null) as MarkdownView | null;
	}
